...
use crate::swagger::{Definitions, Paths, get_swagger};
use crate::yaml::{Config, PathsConfig, RequestConfig, get_yaml};
...
#[napi]
pub fn run(config: String) -> String {
  let cfg: Config = serde_yaml::from_str(&config).unwrap();

  let request = cfg.request.unwrap_or(RequestConfig { headers: None });
  let headers = request.headers.unwrap_or_default();
  let mut swagger = get_swagger(cfg.url, headers);

  let mut paths = swagger.paths.clone();
  if let Some(mut config_paths) = cfg.paths {
    let remove = config_paths.shift_remove("__remove");
    rename_paths(&mut paths, config_paths);
    if let Some(paths_to_remove) = remove {
  ...
  if let Some(mut def) = cfg.definitions {
      ...
      if let Some(parameters) = method_obj.get("parameters") {
        ...
        for parameter in parameters_obj {
          ...
          if let Some(schema) = parameter_obj.get("schema")
            && let Some(original_ref) = find_original_ref(schema)
          {
      ...
      match str_type {
        "array" => {
          if let Some(items) = obj.get("items")
            && let Some(original_ref) = items.get("originalRef")
          {
            return Some(original_ref.as_str().unwrap().to_string());
          }
        }
        "object" => {
          if let Some(additional_properties) = obj.get("additionalProperties")
            && let Some(original_ref) = additional_properties.get("originalRef")
          {
            return Some(original_ref.as_str().unwrap().to_string());
          }
        }
...
fn update_required(definitions: &mut Definitions) {
  for (_key, definition) in definitions.iter_mut() {
    ...
    }) && let Some(properties) = definition.get("properties")
    {
      definition.insert(
        "required".to_string(),
        properties
...
fn generate_operation_id(verb: &str, url: &str) -> String {
  let mut op: String;
  match verb {
